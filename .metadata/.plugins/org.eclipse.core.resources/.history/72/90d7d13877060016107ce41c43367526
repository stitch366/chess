...
public class ChessUtil {
	...
	public boolean canKingSideCastle(){
		...
		boolean areSpacesNotSafeForKing = areSpacesUnderAttack(KingSide.getSpacesKingToches(), king.getColor());
		if((!areSpacesNotSafeForKing)&& (!rook.hasMoved) && (!king.hasMoved) && (!isRookCaptured) && areSpacesEmpty)
		{
			return true;
		}
		else
	...
	private boolean areSpacesUnderAttack(ArrayList<String> spaces, PeiceColor p)
	{
		ArrayList<String> enemyMoves = GetPossEnemyMoves(p);
		
		for(int x =0; x < spaces.size(); x++)
		{
			if(enemyMoves.contains(spaces.get(x)))
			{
				return true;
			}
		}
	...
	public boolean canQueenSideCastle(){
		...
		boolean areSpacesNotSafeForKing = areSpacesUnderAttack(QueenSide.getSpacesKingToches(), king.getColor());
		if((!areSpacesNotSafeForKing)&&(!rook.hasMoved) && (!king.hasMoved) && (!isRookCaptured) && areSpacesEmpty)
		{
			return true;
		}
